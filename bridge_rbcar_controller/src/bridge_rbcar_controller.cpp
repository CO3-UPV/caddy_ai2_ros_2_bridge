...
class MinimalNode : public rclcpp::Node
{
  private:
    ...
    //rclcpp::Publisher<std_msgs::msg::String>::SharedPtr _pub_2_;
    ...
    void timer_callback()
    {
      if(_ros_1_new_message_1_){
        ...
        mtx_1.lock();
        message.data = _pub_json_1_["speed"];
        _ros_1_new_message_1_ = false;
        mtx_1.unlock();
        _pub_1_->publish(message);
      }
      if(_ros_1_new_message_2_){
        auto message = sensor_msgs::msg::Imu();
        mtx_2.lock();
        message.orientation.x = _pub_json_2_["orientation"]["x"];
        message.orientation.y = _pub_json_2_["orientation"]["y"];
        message.orientation.z = _pub_json_2_["orientation"]["z"];
        message.orientation.w = _pub_json_2_["orientation"]["w"];
        message.angular_velocity.x = _pub_json_2_["angular_velocity"]["x"];
        message.angular_velocity.y = _pub_json_2_["angular_velocity"]["y"];
        message.angular_velocity.z = _pub_json_2_["angular_velocity"]["z"];
        message.linear_acceleration.x = _pub_json_2_["linear_acceleration"]["x"];
        message.linear_acceleration.y = _pub_json_2_["linear_acceleration"]["y"];
        message.linear_acceleration.z = _pub_json_2_["linear_acceleration"]["z"];
        _ros_1_new_message_2_ = false;
        mtx_2.unlock();
        _pub_2_->publish(message);
      }
    }
  ...
  public:
        ...
        : Node("bridge_rbcar_controller_node", options)
    {
        ...
        this->declare_parameter("pub_topic_2", "speed");
        this->declare_parameter("ros_2_server_port_2", 8889);
